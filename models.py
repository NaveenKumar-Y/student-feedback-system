...
class Models:
    ...
    def __init__(self):
        ...
        df = pd.read_csv(path)
        df = df.dropna()
        # self.x = df['sentences'][:1200]
        # self.y = df['sentiments'][:1200]
        # self.x1 = df['sentences'][1200:]
        # self.y1 = df['sentiments'][1200:]
        X=df['sentences']
        y=df['sentiments']
        self.x, self.x1, self.y, self.y1 = train_test_split(X, y, test_size=0.20)
